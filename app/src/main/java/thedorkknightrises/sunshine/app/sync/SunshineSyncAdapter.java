package thedorkknightrises.sunshine.app.sync;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    public static final String ACTION_DATA_UPDATED =
            "com.sunshine.app.ACTION_DATA_UPDATED";
    ...
    private int wearHigh = 0;
    private int wearLow = 0;
    ...
    private void updateWearable() {
        ...
        putDataMapReq.getDataMap().putInt(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID, wearId);
        putDataMapReq.getDataMap().putString(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP, Integer.toString(wearHigh));
        putDataMapReq.getDataMap().putString(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP, Integer.toString(wearLow));
        putDataMapReq.getDataMap().putLong("Time", System.currentTimeMillis()); //send timestamp
        PutDataRequest putDataReq = putDataMapReq.asPutDataRequest();
        Wearable.DataApi.putDataItem(mGoogleApiClient, putDataReq)
                .setResultCallback(new ResultCallback<DataApi.DataItemResult>() {
                    @Override
                    public void onResult(@NonNull DataApi.DataItemResult dataItemResult) {
                        Log.d("Update Wearable", dataItemResult.toString());
                    }
                });
        Log.d("Update Wearable", "Completed");
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            for (int i = 0; i < weatherArray.length(); i++) {
                ...
                high = (int) temperatureObject.getDouble(OWM_MAX);
                low = (int) temperatureObject.getDouble(OWM_MIN);

                if (i == 0) {
                    wearId = weatherId;
                    wearHigh = high;
                    wearLow = low;
                    wearDesc = description;
                }
